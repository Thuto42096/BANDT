...
class POSHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/inventory':
            self.wfile.write(json.dumps(self.get_inventory()).encode())
        elif path == '/api/credit-score':
            self.wfile.write(json.dumps(self.get_credit_score()).encode())
        elif path == '/api/sales-history':
            self.wfile.write(json.dumps(self.get_sales_history()).encode())
...
if __name__ == '__main__':
    ...
    server = HTTPServer(('localhost', 5000), POSHandler)
